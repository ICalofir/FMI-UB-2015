...
class ShallowNet():
    ...
    class fully_connected:
        ...
        def relu_backward(self, dout):
            ...
            ####################################################################
            out_relu = np.maximum(self.h, 0)
            out_relu[out_relu > 0] = 1
            out_relu[out_relu <= 0] = 0
            dx = dout * out_relu
            ####################################################################
            #                         END OF YOUR CODE                         #
            ####################################################################
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        * gradientii: un dictionar care mapeaza pentru fiecare parametru gradientul acelui parametru. 
        ...
        # cross-entropia.                                                           #
        #############################################################################
        y_hot = np.zeros((y.shape[0], self.num_classes))
        y_hot[np.arange(y.shape[0]), y] = 1
        y_ = softmax(scores)

        regularization = reg * (np.sum(np.square(fc1_w)) + np.sum(np.square(fc2_w)))
        loss = (1 / batch_size) * - np.sum(np.multiply(y_hot, np.log(y_))) + regularization
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
        ...
        #############################################################################
        delta_2 = y_ - y_hot
        dx, dw, db = self.fc2.backward(delta_2)
        grads['fc2_w'] = (1 / batch_size) * dw + 2 * reg * fc2_w
        grads['fc2_b'] = (1 / batch_size) * db
        dx, dw, db = self.fc1.backward(dx)
        grads['fc1_w'] = (1 / batch_size) * dw + 2 * reg * fc1_w
        grads['fc1_b'] = (1 / batch_size) * db
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
    ...
    def train(self, X, y, X_val, y_val,
        ...
        for it in range(num_iters):
            ...
            #########################################################################
            # !!! e o problema cu vectorii din numpy
            # a = a - b e diferit de a -= b !!!
            self.params['fc1_w'] -= learning_rate * grads['fc1_w']
            self.params['fc1_b'] -= learning_rate * grads['fc1_b']
            self.params['fc2_w'] -= learning_rate * grads['fc2_w']
            self.params['fc2_b'] -= learning_rate * grads['fc2_b']
            #########################################################################
            #                             END OF YOUR CODE                          #
            #########################################################################
