...
isBloodOrange (Orange variety nr) | elem variety tr = True
                                  | otherwise = False
...
bloodOrangeSegments (x:xs) | isBloodOrange x = 1 + bloodOrangeSegments xs
                           | otherwise = bloodOrangeSegments xs

-- 3.
worms :: [Fruit] -> Int
worms [] = 0
worms (x:xs) = case x of
                (Apple variety True) -> 1 + worms xs
                (Apple variety False) -> worms xs
                (Orange _ _) -> worms xs
...
eval e (p :&: q)      =  eval e p && eval e q
eval e (p :->: q)     =  eval e p <= eval e q
eval e (p :<->: q)    =  (eval e p <= eval e q) && (eval e q <= eval e p)
...
-- 5. 
tautology :: Prop -> Bool
tautology p = and [eval x p | x <- (envs (names p))]
...
equivalent p q = and [x == y | (x, y) <- ([(eval x p, eval x q) | x <- (envs (nub (names p ++ names q)))])]
...
subformulas :: Prop -> [Prop]
subformulas p = nub (subformulas' p)
