...
lungimePlus nr n = (replicate n 0) ++ nr
...
normalizeazaLungime (nr1, nr2) | length nr1 > length nr2 = (nr1, (lungimePlus nr2 (length nr1 - length nr2)))
                               | otherwise = ((lungimePlus nr1 (length nr2 - length nr1)), nr2)
...
lte xs ys = lteN (fst (normalizeazaLungime (xs, ys))) (snd (normalizeazaLungime (xs, ys)))
...
propag :: Int -> (Cifra, [Cifra]) -> (Cifra, [Cifra])
propag nr (c, cs) = (((c + nr) `div` 10), (((c + nr) `mod` 10) : cs))

propagaFold' :: [Int] -> (Cifra, [Cifra])
propagaFold' nr = foldr propag (0, []) nr
...
propagaFold nr = (fst (propagaFold' nr)) : (snd (propagaFold' nr))
...
plus n1 n2 = propagaFold (plusLista (fst(normalizeazaLungime (n1,n2))) (snd(normalizeazaLungime (n1,n2))))
...
minus n1 n2 
    | lte n2 n1 = propagaFold (minusLista (fst(normalizeazaLungime (n1,n2))) (snd(normalizeazaLungime (n1,n2))))
    | otherwise = error "Numere negative neimplementate"
...
mulC c nr = propagaFold (map (\x -> x * c) nr)
...
-- (32 de simboluri)
mulRec :: Numar -> Numar -> [Numar]
mulRec [] ys = []
mulRec (x:xs) ys = [(mulC x ys)] ++ (mulRec xs ys)

mul :: Numar -> Numar -> Numar
mul nr1 nr2 = propagaFold (mulRec (reverse nr1) nr2)
